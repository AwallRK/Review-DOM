let arrPage = ["login-form", "regis-form", "content-section", "form-section"];
let page = "";
let userDB = [];
let itemDB = [
  {
...
function showPage(page) {
  ...
  for (let i = 0; i < arrPage.length; i++) {
    if (arrPage[i] === page) {
      document.getElementById(arrPage[i]).style.display = "block";
    } else {
...
function handleRegis() {
  const user = document.getElementById("username-input-regis");
  const password = document.getElementById("password-input-regis");
...
function handleLogin() {
  const user = document.getElementById("username-input-login");
  ...
  if (user.value === "" || password.value === "") {
    ...
    for (let i = 0; i < userDB.length; i++) {
      ...
      if (perUser.user === user.value && perUser.password === password.value) {
        found = true;
      }
    }
    if (!found) {
      alert(
        "Username / Password is invalid, please check your username/password"
      );
      user.value = "";
      password.value = "";
      showPage("login-form");
...
function handleForm() {
  const name = document.getElementById("nameDigimon-input");
  const img = document.getElementById("ImgUrl-input");
  const level = document.getElementById("Level-input");
  if (name.value === "" || img.value === "" || level.value === "") {
    alert("Please enter all the field");
    name.value = "";
    img.value = "";
    level.value = "";
    showPage("form-section");
  } else {
    let digimonObj = {
      name: name.value,
      img: img.value,
...
function load() {
  ...
  for (let i = 0; i < itemDB.length; i++) {
    let perDigimon = itemDB[i];
    let item = document.createElement("div");
    item.classList.add("item");
    item.innerHTML = `
    <img src="${perDigimon.img}" alt="">
    <h3>${perDigimon.name}</h3>
    <p>${perDigimon.level}</p>
    `;
